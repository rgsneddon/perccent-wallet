...
void main() {
  test('version is 1.1.7 and macos installer metadata is real', () {
    expect(File('pubspec.yaml').readAsStringSync(), contains('version: 1.1.7+12'));
    final metaFile = File('installer/macos/perccent-wallet-v1.1.7-macos.json');
    ...
    expect(meta['name'], 'perccent-wallet-v1.1.7-macos-setup.zip');
  ...
  test('downloads page lists macOS v1.1.7 card with matching sha256', () {
    ...
    expect(html, contains('perccent-wallet-v1.1.7-macos-setup.zip'));
    final meta = jsonDecode(
      File('installer/macos/perccent-wallet-v1.1.7-macos.json').readAsStringSync(),
  ...
  test('README and RELEASE_NOTES document macOS v1.1.7', () {
    ...
    expect(notes, contains('perccent-wallet-v1.1.7-macos-setup.zip'));
    final meta = jsonDecode(
      File('installer/macos/perccent-wallet-v1.1.7-macos.json').readAsStringSync(),
    ) as Map<String, dynamic>;
